...
Stochasticity is needed to encourage exploration and mimic random sampling from given categorical distribution. We can inject this stochasticity perturbing the logits by adding a noise $\epsilon$ value before applying softmax.
...
Gumble noise $\epsilon$ is sampled from standard [***Gumbel distribution***](https://en.wikipedia.org/wiki/Gumbel_distribution). We can draw such noise samples by first computing $u \sim Uniform(0, 1)$ and then computing $\epsilon = -\log(-\log(u))$ .
